...
namespace SkylineNModFilter
{
    internal sealed class ToolArguments
    {
        ...
        public ReplicateAnnotationOptions ReplicateAnnotationOptions { get; private set; }
        ...
        public static ToolArguments Parse(string[] args)
        {
            ...
            for (var i = 0; i < args.Length; i++)
            {
                ...
                else if (args[i] == "--import-replicate-annotations") importAnnotations = true;
            }
            ...
            if (!reorder && !groupedMissingness && !importAnnotations && (hasHeader || !string.IsNullOrWhiteSpace(manifestPath))) throw new ArgumentException("Metadata file options require replicate ordering, grouped missingness filtering, or replicate annotation import.");
            ...
            if (importAnnotations && (!hasHeader || string.IsNullOrWhiteSpace(manifestPath))) throw new ArgumentException("Replicate annotation import requires a metadata file with a header.");
            result.ReplicateAnnotationOptions = importAnnotations ? ReplicateAnnotationOptions.EnabledFor(manifestPath) : ReplicateAnnotationOptions.Disabled;
            return result;
        }
    }
}
