namespace SkylineNModFilter.Tests
{
    internal static class CompletionMessageTests
    {
        public static void Run()
        {
            ...
            var annotated = CompletionMessage.Build(new FilterResult { OutputPath = "x.sky", ReplicateAnnotationResult = new ReplicateAnnotationResult { AnnotatedReplicates = 8, UnannotatedReplicates = 2, UnmatchedMetadataRows = 3, DuplicateMetadataKeys = 1, IgnoredMetadataRows = 4, MissingDefinitions = new[] { "Condition", "Batch" }, ExistingDefinitions = new[] { "Subject" } } });
            foreach (var expected in new[] { "Replicates annotated: 8", "Replicates left unannotated: 2", "Unmatched metadata rows: 3", "Duplicate metadata keys: 1", "Ignored metadata rows: 4", "Annotation definitions created: Condition, Batch", "Existing annotation definitions used: Subject" }) TestAssert.True(annotated.Contains(expected), "Annotation completion message missing: " + expected);
        }
    }
}
